<template>
  <div>
    <el-form :inline="true" :model="formInline" class="demo-form-inline">
      ...
      <el-form-item label="状态">
        <el-select v-model="formInline.statusInput" placeholder="全部">
          ...
          <el-option label="已审核,待开票" value="22"></el-option>
          <el-option label="待审核,待开票" value="11"></el-option>
          <el-option label="已审核,未开票" value="21"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item>
    ...
    >
      ...
      <el-table-column prop="check_status,invoice_status" label="状态">
...
export default {
  ...
  methods: {
    onSubmit() {
      this.invoiceStatusInput = this.formInline.statusInput.slice(1, 2);
      this.checkStatusInput = this.formInline.statusInput.slice(0, 1);
      if (this.formInline.statusInput === "") {
        ...
        this.tableDataSearch = this.tableData.filter((item) => {
          return (
            item.invoice_status.indexOf(this.invoiceStatusInput) !== -1 &&
            item.check_status.indexOf(this.checkStatusInput) !== -1
          );
        });
      } else {
        this.tableDataSearch = this.tableData.filter((item) => {
    ...
    goWait(id, invoice_status, check_status) {
      invoice_status == "2" && check_status == "2"
        ? this.$router.push({ path: "/wait1", query: { id: id } })
        : this.$router.push({ path: "/wait2", query: { id: id } });
    },
    ...
    async invoiceList(time) {
      ...
      this.$axios({
        method: "post",
        url: "https://wemall.minephone.com/Invoice/api/public/index.php/api/admin/invoiceList",
        data: data,
      }).then((result) => {
